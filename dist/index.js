...
if (!CU_TOKEN)
    throw new Error("CU_TOKEN env is required");
const listId = process.env.INPUT_LIST_ID; // from action input
const mode = (process.env.INPUT_MODE ?? "auto");
...
const payload = JSON.parse((0,external_node_fs_namespaceObject.readFileSync)(payloadPath, "utf8"));
// ---------------------------------------------------------------------------
// Simple axios wrapper
const cu = lib_axios.create({
...
async function createTask(title, commitUrl) {
    try {
...
async function createSubTask(parentId, title, commitUrl) {
    try {
